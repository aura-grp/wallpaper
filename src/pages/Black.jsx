  ...
  const fetchImages = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `https://api.unsplash.com/search/photos?query=black_wallpaper&per_page=12&page=${page}&client_id=${ACCESS_KEY}`
      );
      ...
      >
        {images.map((img) => (
          <div key={img.id} className="mb-4 overflow-hidden rounded-lg shadow-lg">
            <img
              src={img.urls.small}
              alt={img.alt_description || "black wallpaper"}
              loading="lazy"
